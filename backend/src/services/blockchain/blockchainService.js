...
import {
  ...
  hashEvidence,
} from "./evidenceCanonicalizer.js";

const ledgerPath = path.resolve(
  path.dirname(fileURLToPath(import.meta.url)),
  "../../../",
  env.blockchainLedgerPath
);

const ledger = new LocalEvidenceLedger(ledgerPath, {
  retentionDays: env.blockchainLedgerRetentionDays,
});
...
export function evidenceStatusPayload(
  accident,
  status = accident.blockchainStatus || "PENDING"
) {
  return {
...
export async function recordEvidence(accident) {
  if (!env.blockchainEnabled) {
    return {
      status: "UNAVAILABLE",
      error: "Blockchain evidence is disabled",
    };
  }

  if (!accident?.evidenceId) {
    throw new Error("Accident has no evidenceId");
  }

  const snapshot = createEvidenceSnapshot(accident);
  const evidenceHash = hashEvidence(snapshot);
  const commitment = await ledger.record(
    accident.evidenceId,
    evidenceHash,
    EVIDENCE_VERSION
  );
...
export async function verifyEvidence(accident) {
  ...
  if (!accident?.evidenceId) {
    return {
      status: "PENDING",
      match: false,
      error: "Evidence is not initialized",
    };
  }
