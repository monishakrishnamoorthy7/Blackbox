...
import {
  ...
  hashEvidence,
} from "../src/services/blockchain/evidenceCanonicalizer.js";
...
function createClockLedger(fileName) {
  const clock = {
    value: new Date("2026-09-01T12:00:00Z"),
  };

  const ledger = new LocalEvidenceLedger(
...
before(async () => {
  tempDir = await mkdtemp(
    path.join(os.tmpdir(), "bike-black-box-ledger-")
  );
});
...
describe("tamper-evident evidence", () => {
  it("produces the same hash for equivalent evidence", () => {
    const reordered = {
      ...
      triggeredSignals: [
        ...baseEvidence.triggeredSignals,
      ].reverse(),
    };

    assert.equal(
      hashEvidence(baseEvidence),
      hashEvidence(reordered)
    );

    assert.equal(
      hashEvidence(baseEvidence),
      hashEvidence(createEvidenceSnapshot(baseEvidence))
    );
  });
  ...
  it("excludes GPS coordinates, personal information, and mutable fields", () => {
    ...
    assert.equal(
      Object.hasOwn(snapshot, "latitude"),
      false
    );

    assert.equal(
      Object.hasOwn(snapshot, "longitude"),
      false
    );

    assert.equal(canonical.includes("12.97"), false);
    assert.equal(
      canonical.includes("should-not-be-included"),
      false
    );
    assert.equal(
      canonical.includes("SOS_TRIGGERED"),
      false
    );
    assert.equal(
      canonical.includes("private"),
      false
    );
  });
  ...
  it("creates a linked append-only ledger and records idempotently", async () => {
    const ledger = new LocalEvidenceLedger(
      path.join(tempDir, "evidence.jsonl")
    );

    const first = await ledger.record(
      baseEvidence.evidenceId,
      hashEvidence(baseEvidence),
      1
    );
    ...
    assert.equal(
      (await ledger.verifyChain()).valid,
      true
    );
  });
  ...
  it("detects a changed accident snapshot during verification", async () => {
    const ledger = new LocalEvidenceLedger(
      path.join(tempDir, "verify.jsonl")
    );
    ...
    const commitment = await ledger.find(
      baseEvidence.evidenceId
    );
  ...
  it("handles an empty ledger and retains records younger than seven days", async () => {
    const { clock, ledger } = createClockLedger(
      "retention-young.jsonl"
    );

    assert.deepEqual(
      await ledger.records(),
      []
    );
